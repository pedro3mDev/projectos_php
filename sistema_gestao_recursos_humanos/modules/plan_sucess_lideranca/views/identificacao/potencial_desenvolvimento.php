        ...
        <div class="row">
            <div class="col-md-10">
                <a class="dashboard-link" href="" style="font-size: 16px; color: #333;">
                    Plano de Sucessão e Liderança / Identificação / Potencial Desenvolvimento
                </a>
            </div>
                    ...
                    <div class="panel-body">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <a href="#" data-toggle="modal" data-target="#modal_add_potencial_desenvolvimento"
                                    class="btn" style="background-color: #007bff; border-color: #007bff; color: white;">
                                    <i class="fa-regular "></i>
                                    Adicionar Potencial Desenvolvimento
                                </a>
                            ...
                            <div class=" col-md-3">
                                <select name="potencial_pd_f" id="potencial_pd_f" class="selectpicker" multiple="true"
                        ...
                        <table class="table dt-table">
                            <thead class="thead-custom">
                                <th>Funcionário</th>
                                <th>Potencial</th>
                                <th>Sugest. Dsenvolvimento</th>
                                <th>Estado</th>
                                <th></th>
                            </thead>
                            <tbody>
                                <?php
                                            foreach($potencial_desenvolvimento as $item) {
                                        ?>
                                <tr>
                                    <td>
                                    ...
                                    <td class="text-capitalize"> <?= $item['sugestao_desenvolvimento']; ?> </td>
                                    <td>
                                        <?php
                                    ...
                                    <td>
                                        ...
                                        <a href="<?php echo admin_url('plan_sucess_lideranca/potencial_desenvolvimento_aprovar/'.$item['id']); ?>"
                                            class="btn btn-success" style="color: white;">
                                            Aprovar
                                        </a>
                                        <a onclick="return confirm('Tens certteza que desejas rejeitar?');"
                                            href="<?php echo admin_url('plan_sucess_lideranca/potencial_desenvolvimento_rejeitar/'.$item['id']); ?>"
                                            class="btn btn-danger" style="color: white;">
                                            Rejeitar
                                        </a>
                                        <?php endif ?>
                                        <a href="<?php echo admin_url('plan_sucess_lideranca/identificacao_visualizar_potencial/'.$item['id'])?>" class="btn btn-success btn-icon">
                                                    <i style="color: white;" class="fa fa-eye"></i>
                                        </a>
                                        <a href="javascript:;" data-id="<?= $item['id'] ?>"
                                            data-funcionario='<?= $item["staff_id"] ?>'
                                            data-potencial="<?= html_entity_decode($item['potencial_id']) ?>"
                                            data-sugestao_desenvolvimento="<?= html_entity_decode($item['sugestao_desenvolvimento']) ?>"
                                            data-aprovadores='<?= html_entity_decode(($item["aprovadores"] ?? "")) ?>'
                                            class="btn btn-default btn-icon btn_edit_potencial_desenvolvimento"
                                            style="background-color: #007bff; border-color: #007bff;">
                                            <i style="color: white;" class="fa fa-edit"></i>
                                        </a>
                                        <a onclick="return confirm('Tens certteza que desejas eliminar este Potencial Desenvolvimento?');"
                                            href="<?php echo admin_url('plan_sucess_lideranca/potencial_desenvolvimento_delete/'.$item['id']); ?>"
...
<div class="modal" id="modal_add_potencial_desenvolvimento" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-md">
        <?= form_open(admin_url('plan_sucess_lideranca/adicionar_potencial_desenvolvimento'), array('method' => 'post')) ?>
        <div class="modal-content ">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                        aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">
                    Adicionar Potencial de Desenvolvimento
                </h4>
            </div>
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="form-group">
                            <label for="company" class="control-label">
                                Sugestão de Desenvolvimento <small class="req text-danger">*</small>
                            </label>
                            <textarea class="form-control" style="min-height: 80px;"
                                name="sugestao_desenvolvimento"></textarea>
                        </div>
                        <div class="form-group">
                            <?php
                            $selectedStaff = '';
...
<?= form_open(admin_url('plan_sucess_lideranca/editar_potencial_desenvolvimento'), array('method' => 'post', 'id' => 'form_edit_potencial_desenvolvimento')) ?>
    ...
    <div class="modal-dialog w-25">
        <div class="modal-content ">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                        aria-hidden="true">&times;</span></button>
                <h4 class="modal-title"> Editar Potencial Desenvolvimento </h4>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-12 text-danger">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" style="min-height: 80px;"
                                name="e_sugestao_desenvolvimento"></textarea>
                        </div>
                        <div class="form-group">
                            <?php
                            $selectedStaff = '';
...
$('.btn_edit_potencial_desenvolvimento').click(function() {
    let funcionario = $(this).attr('data-funcionario');
    let potencial = $(this).attr('data-potencial');
    let sugestao_desenvolvimento = $(this).attr('data-sugestao_desenvolvimento');
    let aprovadores = $(this).attr('data-aprovadores');

    let id = $(this).attr('data-id');

    let url = "<?= admin_url('plan_sucess_lideranca/editar_potencial_desenvolvimento') ?>/" + id;
    $('#form_edit_potencial_desenvolvimento').attr('action', url);

    $('select[name="e_funcionario"]').selectpicker('val', funcionario);
    $('select[name="e_potencial"]').val(potencial);
    $('textarea[name="e_sugestao_desenvolvimento"]').text(sugestao_desenvolvimento);
...
$('#potencial_pd_f').change(function() {
    var tabela = $('.dt-table').DataTable();
