			...
			<div class="panel-body">
				...
				<div class="row">
					<div class="col-md-12">
						...
						<?php } else { 
							if(($share_to_me == 1 && $parent_id == 0) || (isset($item) && $item->filetype == 'folder' && !check_share_permission($parent_id, 'upload_only', 'customer'))){ ?>
								<?php echo render_input('search', '', '', 'text', ['placeholder' => _l('dmg_search_name_tag_etc')], [], 'pull-right default-tool'); ?>
							<?php } ?>
						<?php } ?>
						<div class="col-md-12">
					...
					<div class="col-md-3 border-right">
						...
						<ul class="list-group list-group-flush list-group-custom" role="tablist">
							<li class="list-group-item list-group-item-action display-flex<?php echo ($share_to_me == 1 ? ' active' : ''); ?>" data-toggle="list" role="tab">
								<a href="<?php echo site_url('document_management/document_management_client?share_to_me=1&id=0'); ?>" class="w100 display-flex">
					...
					<div class="col-md-9">
						...
						<?php } else { ?>
							...
							<?php if($share_to_me == 1){ ?>
								<?php if($parent_id > 0){ ?>
									<div class="row">
										<div class="col-md-12">
											<?php 
											$html_breadcrumb = '';
											$data_breadcrumb = $this->document_management_model->breadcrum_array2($parent_id, 'customer');
											foreach ($data_breadcrumb as $key => $value) {
												$html_breadcrumb = '<li class="breadcrumb-item"><a href="'.site_url('document_management/document_management_client?share_to_me=1&id='.$value['id']).'">'.$value['name'].'</a></li>'.$html_breadcrumb;
											}
											?>
											<nav aria-label="breadcrumb">
												<ol class="breadcrumb">
								...
								if(check_share_permission($parent_id, 'upload_only', 'customer')){ ?>	
									<div class="file-form-group file-form">
										<?php echo form_open_multipart(site_url('document_management/document_management_client/upload_file/'.$parent_id.'/share_to_me'),array('id'=>'form_upload_file')); ?>              
										<input type="file" id="files" name="file[]" multiple="">
										<div class="file-form-preview hide">
											<ul class="selectedFiles list-group list-group-flush mtop15" id="selectedFiles"></ul>
											<hr>
									...
									if($parent_id == 0 || (is_numeric($parent_id) && $parent_id > 0 && dmg_get_file_type($parent_id) == 'folder')){
										...
										if(check_share_permission($parent_id, 'editor', 'customer')){ ?>	
											<div class="file-form-group file-form">
												<?php echo form_open_multipart(site_url('document_management/document_management_client/upload_file/'.$parent_id.'/share_to_me'),array('id'=>'form_upload_file')); ?>              
												<input type="file" id="files" name="file[]" multiple="">
												<div class="file-form-preview hide">
...
<div class="modal remider" id="remider" tabindex="-1" role="dialog">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<?php echo form_open(site_url('document_management/document_management_client/create_remider/'.$parent_id),array('id'=>'form_create_remider')); ?>              
			<div class="modal-body">
				<div class="row">
					<?php
					$redirect_type = '';
...
<?php 
require 'modules/document_management/assets/js/clients/file_managements/file_management_js.php';
if($edit == 1){
	require 'modules/document_management/assets/js/clients/file_managements/edit_file_js.php';
}
if(isset($item) && $item->filetype != 'folder' && $edit != 1){
	require 'modules/document_management/assets/js/clients/file_managements/file_detail_js.php';
}
?>
