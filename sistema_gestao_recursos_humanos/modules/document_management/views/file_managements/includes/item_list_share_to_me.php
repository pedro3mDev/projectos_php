	<table class="table table-items scroll-responsive no-mtop">
		...
		<tbody>
			...
			foreach ($child_items as $key => $value) { 
				...
				$a1 = '<a href="'.admin_url('document_management?share_to_me=1&id='.$value['id']).'" >';
				$a2 = '</a>';
				?>
				<tr>
					<td>
					...
					<td>
						...
						$editor_permit = check_share_permission($value['id'], 'editor');
						$viewer_permit = check_share_permission($value['id'], 'viewer');
								...
								<ul class="dropdown-menu" aria-labelledby="dropdown_menu_<?php echo htmldecode($value['id']); ?>">
									...
									if($value['filetype'] == 'folder'){ 
										...
										if(!check_file_locked($value['id']) && $editor_permit){ ?>
											<li class="no-padding">
												<a href="<?php echo admin_url('document_management?share_to_me=1&id='.$value['id'].'&edit=1') ?>" data-name="<?php echo htmldecode($value['name']); ?>"><?php echo _l('dmg_edit_metadata') ?></a>											
											</li>
										<?php } 
									}
									if($editor_permit){ ?>
