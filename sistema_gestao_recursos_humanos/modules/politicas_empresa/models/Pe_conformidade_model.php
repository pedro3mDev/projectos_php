...
class Pe_conformidade_model extends CI_Model
{
    ...
    {
        $query = $this->db->get('pe_conformidade')->num_rows();
        return $query;
    }
    public function create($data)
    {
        $this->Pe_politica_empresa_model->first_staff($data['staff_conformidade_id']);
        $this->Pe_politica_model->first($data['politica_id']);

        return $this->db->insert('pe_conformidade', $data);
    }
    ...
    public function get()
    {
        $this->db->select('
            ...
            pe_politica.titulo
        ');
        $this->db->from('pe_conformidade');
        $this->db->join('staff', 'staff.staffid = pe_conformidade.staff_id', 'left');
        $this->db->join('staff as staff_c', 'staff_c.staffid = pe_conformidade.staff_conformidade_id', 'left');
        $this->db->join('pe_politica', 'pe_politica.id = pe_conformidade.politica_id', 'left');
    ...
    public function first($id)
    {
        ...
        $this->db->join('staff as staff_c', 'staff_c.staffid = pe_conformidade.staff_conformidade_id', 'left');
        $this->db->join('pe_politica', 'pe_politica.id = pe_conformidade.politica_id', 'left');
        $this->db->where('pe_conformidade.id', $id);
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update('pe_conformidade', $data);
    ...
    public function delete($id)
    {
        $first = $this->first($id);
        $this->db->where('id', $id);
        $this->db->delete('pe_conformidade');
        return $first;
    }
    public function getTotalConformidadeByStatus($status = 'pendente')
    {
        ...
        if ($this->db->table_exists(db_prefix() . 'pe_conformidade')) {
            $this->db->from(db_prefix() . 'pe_conformidade');
    ...
    public function get_status_totals_with_percentages()
    {
        ...
        status,
        COUNT(*) AS total,
        ROUND((COUNT(*) * 100 / (SELECT COUNT(*) FROM " . db_prefix() . "pe_conformidade)), 2) AS percentual
    ");
        $this->db->from(db_prefix() . 'pe_conformidade');
