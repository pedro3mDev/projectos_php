...
if (approvify_categoria_exists(1) == 0){
    $CI->db->query('INSERT INTO `'.db_prefix().'approvify_approval_categories` (`category_name`,`category_description`) VALUES ("Requisição de Viagem","Requisição de Viagem");
    ');
  }
  else {
    $CI->db->query('UPDATE `'.db_prefix().'approvify_approval_categories` SET `category_name` = "Requisição de Viagem", `category_description` = "Requisição de Viagem" WHERE `id` = 1;');
  }
