...
class Hr_profile extends AdminController
{
	...
	public function dashboard()
	{
		...
		$data['total_funcionarios'] = $this->hr_profile_model->total_funcionarios();
		$data['total_contractos'] = $this->hr_profile_model->total_contractos();
		$data['total_onbording'] = $this->hr_profile_model->total_onbording();
		$data['total_treinamentos'] = $this->hr_profile_model->total_treinamentos();

		$data['dasboard_func_genero'] = $this->hr_profile_model->dasboard_func_genero();

		$dashboard_onbording = $this->dashboard_onbording_c();
		$data['dashboard_onbording_nomes'] = $dashboard_onbording['nomes'];
		$data['dashboard_onbording_persentagens'] = $dashboard_onbording['persentagens'];

		$data['dashboard_usuarios_anuais_actual'] = $this->hr_profile_model->dashboard_usuarios_anuais();
		$ano_anterior = date('Y') - 1;
		$data['dashboard_usuarios_anuais_anterior'] = $this->hr_profile_model->dashboard_usuarios_anuais($ano_anterior);



		$data['total_valid_contracts'] = $this->hr_profile_model->total_status_contracts();
		$data['total_finish_contracts'] = $this->hr_profile_model->total_status_contracts("finish");
		$data['total_draft_contracts'] = $this->hr_profile_model->total_status_contracts("draft");
		$data['total_invalid_contracts'] = $this->hr_profile_model->total_status_contracts("invalid");

		$year = date('Y');
		$data['year1'] = intval($year);
		$data['month1'] = $this->hr_profile_model->total_trainings_per_month($year);
		$data['year2'] = $year - 1;
		$data['month2'] = $this->hr_profile_model->total_trainings_per_month($year - 1);
		
		$this->load->view('hr_profile_dashboard', $data);
	}

	public function dashboard_onbording_c()
	{
		$staffs = $this->hr_profile_model->dashboard_onbording();

		$dados_nome = "[";
		$dados_persentagem = "[";
		foreach ($staffs as $staff) {
			$dados_nome .= "'" . $staff['firstname'] . ' ' . $staff['lastname'] . "',";
			$dados_persentagem .= "'" . round((float)$this->get_percent_complete($staff['staffid']), 2) . "',";
		}
		$dados_nome .= "]";
		$dados_persentagem .= "]";
		return [
			'nomes'  => $dados_nome,
			'persentagens'  => $dados_persentagem,
		];
	}
	...
	public function documentacao()
	{
		...
		$data['title'] = 'documentacao';
		$this->load->view('documentacao', $data);
	}
	...
	public function acompanhamento()
	{
		...
		$this->load->view('acompanhamento', $data);
	}
	...
	public function organizational_chart()
	{
		...
		if (!is_admin() && !has_permission('staffmanage_orgchart', '', 'view')) {
			...
			$data['deparment_chart'] = json_encode($this->hr_profile_model->get_data_chart());
		}
		...
		$data['staffs'] = $this->db->get_where('staff', ['active' => 1])->result_array();

		$this->load->view('organizational/organizational_chart', $data);
	}

	public function editar_responsavel_conselho ($id) {
		$this->http_method('POST');
		$this->hr_profile_model->conselho_first($id);

		$data = [
			'gerente_id' => $this->input->post('e_staff') ?? '',
        ];

		$this->hr_profile_model->conselho_update($data, $id);
			set_alert('success',"Actualizado com sucesso");
			redirect('hr_profile/organizational_chart');
	}
	...
	public function setting()
	{
		...
		if ($data['group'] == '') {
			...
			$data['group'] = 'pelorios';
			$data['contract'] = $this->hr_profile_model->get_contracttype();
			$data['tabela'] = $this->db->get('tblhr_pelorios')->result_array();
		} elseif ($data['group'] == 'direcoes') {
			$data['group'] = 'direcoes';
			$data['contract'] = $this->hr_profile_model->get_contracttype();
			$data['tabela'] = $this->db->get('tblhr_direcoes')->result_array();
			$data['opt_pelorios'] = $this->db->get('tblhr_pelorios')->result_array();
		} elseif ($data['group'] == 'departments') {
			$data['group'] = 'departments';
			$data['contract'] = $this->hr_profile_model->get_contracttype();
			$data['dpt'] = $this->db->get('tbldepartments')->result_array();
			$data['opt_direcoes'] = $this->db->get('tblhr_direcoes')->result_array();
		} elseif ($data['group'] == 'seccoes') {
			$data['group'] = 'seccoes';
			$data['contract'] = $this->hr_profile_model->get_contracttype();
			$data['tabela'] = $this->db->get('tblhr_seccoes')->result_array();
			$data['opt_dpt'] = $this->db->get('tbldepartments')->result_array();
		} elseif ($data['group'] == 'conselho') {
			$data['group'] = 'conselho';
			$data['contract'] = $this->hr_profile_model->get_contracttype();
			$data['tabela'] = $this->db->get(db_prefix() . 'hr_conselhos')->result_array();
			$data['opt_dpt'] = $this->db->get('tbldepartments')->result_array();
		}
		...
		if ($this->input->is_ajax_request()) {
			...
			foreach ($rResult as $aRow) {
				...
				if (has_permission('hrm_setting', '', 'edit')) {
					if (!empty($this->input->get('config'))) {
						$options = icon_btn('#', 'fa-regular fa-pen-to-square', 'btn-default', [
							'title' => _l('hr_edit'),
							'onclick' => 'hr_profile_permissions_update_conf(' . $aRow['staffid'] . ', ' . $aRow['role'] . ', ' . $not_hide . '); return false;',
						]);
					}
					else {
						$options = icon_btn('#', 'fa-regular fa-pen-to-square', 'btn-default', [
	...
	public function permission_modal()
	{
		...
		if ($this->input->post('slug') === 'update') {
			...
			if ($this->input->post('config')) {
				$this->load->view('includes/permissions_config', $data);
			}
			else {
	...
	public function hr_profile_update_permissions($id = '')
	{
		...
		// return 1;
		if (!empty($this->input->get('config'))) {
			redirect(admin_url('configuracoes/sistema?group=permissoes'));
		}
		else {
	...
	public function new_member()
	{
		...
		$data['opt_pelorios'] = $this->db->get(db_prefix() . 'hr_pelorios')->result_array();
		$data['opt_direcoes'] = $this->db->get(db_prefix() . 'hr_direcoes')->result_array();
		$data['opt_seccoes'] = $this->db->get(db_prefix() . 'hr_seccoes')->result_array();
		$data['opt_conselhos'] = $this->db->get(db_prefix() . 'hr_conselhos')->result_array();
	...
	public function type_of_training($id = '')
	{
		if ($this->input->post()) {
			...
			$this->form_validation->set_rules('name', 'Nom', 'required', ['required' => 'Preencha o campo {field}']);
			if (!$this->form_validation->run()) {
				set_alert('danger', 'Preencha todos os campos obrigatórios');
				return redirect(admin_url('hr_profile/setting?group=type_of_training'));
			}
	...
	{
		$nome = $this->input->get('name_dpt');
		$email = $this->input->get('email_dpt');
		$data['name'] = $nome;
		$data['email'] = $email;
		$data['direcoes_id'] = $this->input->get('direcao_id');
		if (empty($this->input->get('name_dpt')) || empty($this->input->get('direcao_id'))) {
			set_alert('danger', 'Preencha todos os campos obrigatórios');
			return redirect(admin_url('hr_profile/setting?group=departments'));
		}
	...
	{
		$this->db->where('departmentid', $id)->delete('tbldepartments');
		return  redirect(admin_url('hr_profile/setting?group=departments'));
	}
	//END DEPARTAMENTOS
	...
	{
		$nome = $this->input->get('name');
		$email = $this->input->get('email');
		$data['nome']        = $nome;
		$data['email']       = $email;
		if (empty($nome)) {
			set_alert('danger', 'Preencha todos os campos obrigatórios');
			return redirect(admin_url('hr_profile/setting?group=conselho'));
		}
		// $data['conselho_id'] = 1;
		$this->db->insert(db_prefix() . 'hr_conselhos', $data);
		set_alert('success', _l('added_successfully', _l('')));
		return  redirect(admin_url('hr_profile/setting?group=conselho'));
	}
	public function edit_conselho()
	{
		$id             = $this->input->get('id');
		$data['nome']   = $this->input->get('e_name');
		$data['email']  = $this->input->get('e_email');
		if (empty($this->input->get('e_name'))) {
			set_alert('danger', 'Preencha todos os campos obrigatórios');
			return redirect(admin_url('hr_profile/setting?group=conselho'));
		}
		try {
			$this->db->where('id', $id);
			$this->db->update(db_prefix() . 'hr_conselhos', $data);
			set_alert('success', _l('Alterado com sucesso', _l('')));
		} catch (\Throwable $th) {
			set_alert('danger', _l('Falha interna', _l('')));
		}
	...
	{
		$this->db->where('id', $id)->delete(db_prefix() . 'hr_conselhos');
		set_alert('success', _l('Eliminado com sucesso', _l('')));
		return  redirect(admin_url('hr_profile/setting?group=conselho'));
	}
	//END CONSELHO
	...
	public function add_pelorios()
	{
		...
		if (empty($nome)) {
			set_alert('danger', 'Preencha todos os campos obrigatórios');
			return redirect(admin_url('hr_profile/setting?group=pelorios'));
		}
		$data['conselho_id'] = 1;
		$this->db->insert('tblhr_pelorios', $data);
		set_alert('success', _l('added_successfully', _l('')));
		return  redirect(admin_url('hr_profile/setting?group=pelorios'));
	}
	public function edit_pelorios()
	{
		$id             = $this->input->get('id');
		$data['nome']   = $this->input->get('e_name');
		$data['email']  = $this->input->get('e_email');
		...
		try {
			$this->db->where('id', $id);
			$this->db->update('tblhr_pelorios', $data);
			set_alert('success', _l('Alterado com sucesso', _l('')));
		} catch (\Throwable $th) {
			set_alert('danger', _l('Falha interna', _l('')));
		}
	...
	public function delete_pelorios($id)
	{
		$this->db->where('id', $id)->delete('tblhr_pelorios');
		set_alert('success', _l('Eliminado com sucesso', _l('')));
		return  redirect(admin_url('hr_profile/setting?group=pelorios'));
	}
	//END PELORIOS
	...
	//DIRECOES
	public function add_direcoes()
	{
		$nome         = $this->input->get('name');
		$email        = $this->input->get('email');
		$pelorio_id   = $this->input->get('pelorio_id');
		$data['nome']       = $nome;
		$data['email']      = $email;
		$data['pelorios_id'] = $pelorio_id;
		if (empty($this->input->get('name')) || empty($this->input->get('pelorio_id'))) {
			set_alert('danger', 'Preencha todos os campos obrigatórios');
			return redirect(admin_url('hr_profile/setting?group=direcoes'));
		}
		$this->db->insert('tblhr_direcoes', $data);
		set_alert('success', _l('added_successfully', _l('')));
		return  redirect(admin_url('hr_profile/setting?group=direcoes'));
	}
	public function edit_direcoes()
	{
		$id             = $this->input->get('id');
		$data['nome']   = $this->input->get('e_name');
		$data['email']  = $this->input->get('e_email');
		$data['pelorios_id'] = $this->input->get('pelorio_id');
		if (empty($this->input->get('e_name')) || empty($this->input->get('pelorio_id'))) {
			set_alert('danger', 'Preencha todos os campos obrigatórios');
			return redirect(admin_url('hr_profile/setting?group=direcoes'));
		}
		...
		try {
			$this->db->where('id', $id);
			$this->db->update('tblhr_direcoes', $data);
			set_alert('success', _l('Alterado com sucesso', _l('')));
		} catch (\Throwable $th) {
			set_alert('danger', _l('Falha interna', _l('')));
		}
	...
	public function delete_direcoes($id)
	{
		$this->db->where('id', $id)->delete('tblhr_direcoes');
		set_alert('success', _l('Eliminado com sucesso', _l('')));
		return  redirect(admin_url('hr_profile/setting?group=direcoes'));
	}
	//END DIRECOES
	...
	public function add_seccoes()
	{
		...
		$data['nome'] = $nome;
		$data['email'] = $email;
		$data['departments_id'] = $this->input->get('departments_id');
		$this->db->insert('tblhr_seccoes', $data);
		set_alert('success', _l('added_successfully', _l('')));
		return  redirect(admin_url('hr_profile/setting?group=seccoes'));
	}
	public function edit_seccoes()
	{
		$id             = $this->input->get('id');
		$data['nome']   = $this->input->get('e_name');
		$data['email']  = $this->input->get('e_email');
		...
		try {
			$this->db->where('id', $id);
			$this->db->update('tblhr_seccoes', $data);
			set_alert('success', _l('Alterado com sucesso', _l('')));
		} catch (\Throwable $th) {
			set_alert('danger', _l('Falha interna', _l('')));
		}
	...
	{
		$this->db->where('id', $id)->delete('tblhr_seccoes');
		set_alert('success', _l('Eliminado com sucesso', _l('')));
		return  redirect(admin_url('hr_profile/setting?group=seccoes'));
	}
	//END SECÇÕES
	...
	// CONSULTAS PARA O FORMULARIO DO FUNCIONARIO

	public function listar_pelorios($conselho_id)
	{
		// Verifica se o ID do conselho foi passado
		if (!$conselho_id) {
			// Retorna um erro se o ID não foi fornecido
			echo json_encode([
				'status' => false,
				'message' => 'ID do conselho não fornecido.'
			]);
			return;
		}
		$pelorios = $this->hr_profile_model->get_pelorios_by_conselho($conselho_id);
	...
	public function listar_direcoes($pelorios_id)
	{
		if (!$pelorios_id) {
			// Retorna um erro se o ID não foi fornecido
			echo json_encode([
				'status' => false,
				'message' => 'ID do pelorios não fornecido.'
			]);
			return;
		}
		$direcoes = $this->hr_profile_model->get_direcoes_by_pelorio($pelorios_id);
	...
	public function listar_departments($direcoes_id)
	{
		if (!$direcoes_id) {
			// Retorna um erro se o ID não foi fornecido
			echo json_encode([
		...
		$departamentos = $this->hr_profile_model->get_departments_by_direcao($direcoes_id);
	...
	public function listar_seccoes($departments_id)
	{
		if (!$departments_id) {
			// Retorna um erro se o ID não foi fornecido
			echo json_encode([
				'status' => false,
				'message' => 'ID do departments não fornecido.'
			]);
			return;
		}
		$seccoes = $this->hr_profile_model->get_seccoes_by_departamento($departments_id);
