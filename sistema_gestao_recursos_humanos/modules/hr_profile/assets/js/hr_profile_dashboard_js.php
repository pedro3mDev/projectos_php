<script>
'use strict';
staff_chart_by_age('staff_chart_by_age', <?php echo new_html_entity_decode($staff_chart_by_age); ?>,
    <?php echo json_encode(_l('hr_staff_chart_by_age')); ?>);
staff_chart_by_age('staff_departments_chart', <?php echo new_html_entity_decode($staff_departments_chart); ?>,
    <?php echo json_encode(_l('hr_chart_by_department')); ?>);
staff_chart_by_age('staff_chart_by_job_positions', <?php echo new_html_entity_decode($staff_chart_by_job_positions); ?>,
    <?php echo json_encode(_l('hr_chart_by_job_positions')); ?>);
...
function staff_chart_by_age(id, value, title_c) {
    Highcharts.setOptions({
        ...
        colors: ['#119EFA', '#ef370dc7', '#15f34f', '#791db2d1', '#DDDF00', '#24CBE5', '#64E572', '#FF9655',
            '#FFF263', '#6AF9C4', '#50B432', '#0d91efc7', '#ED561B'
        ]
    });
    ...
    Highcharts.chart(id, {
        ...
        tooltip: {
            pointFormat: '<span style="color:{series.color}">' +
                <?php echo json_encode(_l('invoice_table_quantity_heading')); ?> +
                '</span>: <b>{point.y}</b> <br/>',
            shared: true
        },
        legend: {
...
function staff_chart_by_fluctuate_according_to_seniority(id, value, title_c) {
    ...
    requestGetJSON('hr_profile/get_chart_senior_staff/' + sort_from + '/' + months_report).done(function(response) {
        ...
        Highcharts.setOptions({
            ...
            colors: ['#119EFA', '#ef370dc7', '#15f34f', '#791db2d1', '#DDDF00', '#24CBE5', '#64E572',
                '#FF9655', '#FFF263', '#6AF9C4', '#50B432', '#0d91efc7', '#ED561B'
            ]
        });
        Highcharts.chart(id, {
            chart: {
            ...
            xAxis: [{
                categories: ['<= 1 <?php echo _l('months'); ?>',
                    '<= 3 <?php echo _l('months'); ?>', '<= 6 <?php echo _l('months'); ?>',
                    '<= 9 <?php echo _l('months'); ?>', '<= 12 <?php echo _l('months'); ?>',
                    '> 12 <?php echo _l('months'); ?>'
                ],
                crosshair: true
            }],
            yAxis: [{ // Primary yAxis
...
function report_by_staffs(id, value, title_c) {
    ...
    requestGetJSON('hr_profile/report_by_staffs').done(function(response) {
        ...
        Highcharts.setOptions({
            ...
            colors: ['#119EFA', '#15f34f', '#ef370dc7', '#791db2d1', '#DDDF00', '#24CBE5', '#64E572',
                '#FF9655', '#FFF263', '#6AF9C4', '#50B432', '#0d91efc7', '#ED561B'
            ]
        });
        Highcharts.chart(id, {
