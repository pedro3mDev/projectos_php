...
(function() {
    ...
    $("body").on('click', '.tagit-close', function() {
        ...
        /*delete tag id*/
        if (tag_id) {
            $.post(admin_url + 'hr_profile/job_position_delete_tag_item/' + tag_id).done(function(
                response) {
    ...
    $("body").on('click', '.add_more_attachments_file', function() {
        ...
        newattachment.find('.' + $.fn.appFormValidator.internal_options.field_wrapper_class).removeClass($
            .fn.appFormValidator.internal_options.field_wrapper_error_class);
        newattachment.find('i').removeClass('fa-plus').addClass('fa-minus');
        newattachment.find('button').removeClass('add_more_attachments_file').addClass(
            'remove_attachment_file').removeClass('btn-success').addClass('btn-danger');
    ...
    var jobPositionServerParams = {
        ...
        "seccao_id": "select[name='seccao']",
        "job_p_id": "[name='job_p_id[]']"
    };
    var table_job_position = $('.table-table_job_position');
    initDataTable(table_job_position, admin_url + 'hr_profile/table_job_position', [0], [0],
        jobPositionServerParams, [1, 'desc']);
    ...
    $('#direcao_hr').on('change', function() {
        alert(122);
        // table_job_position.DataTable().ajax.reload().columns.adjust().responsive.recalc();
    });
    $('#department_hr').on('change', function() {
        table_job_position.DataTable().ajax.reload().columns.adjust().responsive.recalc();
    });
    $('#seccao_hr').on('change', function() {
        table_job_position.DataTable().ajax.reload().columns.adjust().responsive.recalc();
    });
...
function view_hr_profilestaff_file_job_position(id, rel_id) {
    ...
    $("#contract_file_data").load(admin_url + 'hr_profile/preview_job_position_file/' + id + '/' + rel_id, function(
        response, status, xhr) {
