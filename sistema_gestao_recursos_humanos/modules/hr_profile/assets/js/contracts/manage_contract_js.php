...
$(function() {
    ...
    initDataTable(table_contract, admin_url + 'hr_profile/table_contract', [0], [0], ContractsServerParams, [1,
        'desc'
    ]);
    ...
    $('#pelorio').on('change', function() {
        table_contract.DataTable().ajax.reload().columns.adjust().responsive.recalc();
    });
    $('#direcao').on('change', function() {
        table_contract.DataTable().ajax.reload().columns.adjust().responsive.recalc();
    });
    $('#departamento').on('change', function() {
        table_contract.DataTable().ajax.reload().columns.adjust().responsive.recalc();
    });
    $('#seccao').on('change', function() {
        table_contract.DataTable().ajax.reload().columns.adjust().responsive.recalc();
    });
...
function init_hrm_contract(id) {
    ...
    load_small_table_item(id, '#hrm_contract', 'hrmcontractid', 'hr_profile/get_hrm_contract_data_ajax',
...
function load_small_table_item(pr_id, selector, input_name, url, table) {
    ...
    if (!$("body").hasClass('small-table')) {
        toggle_contract_view(table, selector);
    }
...
function view_hrmstaff_file(id, rel_id) {
    ...
    $("#contract_file_data").load(admin_url + 'hr_profile/hrm_file_contract/' + id + '/' + rel_id, function(response,
        status, xhr) {
        if (status == "error") {
