...
$(function() {
    ...
    var StaffServerParams = {
        ...
        "conselho": "select[name='conselho']",
        "pelorio": "select[name='pelorio']",
    ...
    $('#conselho_staff').on('change', function() {
        table_staff.DataTable().ajax.reload().columns.adjust().responsive.recalc();
    });
    $('#pelorio_staff').on('change', function() {
        table_staff.DataTable().ajax.reload().columns.adjust().responsive.recalc();
    });
    $('#direcao_staff').on('change', function() {
        table_staff.DataTable().ajax.reload().columns.adjust().responsive.recalc();
    });
    $('#departamento_staff').on('change', function() {
        table_staff.DataTable().ajax.reload().columns.adjust().responsive.recalc();
    });
    $('#seccao_staff').on('change', function() {
        table_staff.DataTable().ajax.reload().columns.adjust().responsive.recalc();
    });
    $.each(StaffServerParams, function() {
        ...
        'createNode': function(node, data) {
            node.on('click', function(event) {
                if (!$(event.target).is('.edge, .toggleBtn')) {
                    ...
                    var newX = window.parseInt((chart_obj.outerWidth(true) / 2) - (this_obj
                        .offset().left - chart_obj.offset().left) - (this_obj
                        .outerWidth(true) / 2));
                    var newY = window.parseInt((chart_obj.outerHeight(true) / 2) - (this_obj
                        .offset().top - chart_obj.offset().top) - (this_obj.outerHeight(
                        true) / 2));
