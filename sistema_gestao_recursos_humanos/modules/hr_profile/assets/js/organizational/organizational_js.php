...
$(function() {
    ...
    initDataTable(table_departments, admin_url + 'hr_profile/get_data_department', [4], [4], LeadsServerParams,
        [2, 'asc']);
...
function zen_unit_chart(department) {
    ...
    $.post(admin_url + 'hr_profile/zen_unit_chart/' + department).done(function(response) {
        ...
        ds = {
            'image': '',
            'name': '<p class="bold" class="zen_unit_chart"><i class="fa fa-sitemap"></i>' + ' ' + response
                .dpm_name + ' </p>',
            'staff_identifi': '',
        ...
        'createNode': function($node, data) {
            $node.on('click', function(event) {
                if (!$(event.target).is('.edge, .toggleBtn')) {
                    ...
                    var newX = window.parseInt(($chart.outerWidth(true) / 2) - ($this.offset()
                        .left - $chart.offset().left) - ($this.outerWidth(true) / 2));
                    var newY = window.parseInt(($chart.outerHeight(true) / 2) - ($this.offset()
                        .top - $chart.offset().top) - ($this.outerHeight(true) / 2));
