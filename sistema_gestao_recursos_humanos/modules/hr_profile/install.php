...
if (!$CI->db->field_exists('direcao_id' ,db_prefix() . 'hr_job_position')) { 
  $CI->db->query('ALTER TABLE `' . db_prefix() . "hr_job_position`
    ADD COLUMN `direcao_id` int(11) NULL
  ;");
}
if (!$CI->db->field_exists('seccao_id' ,db_prefix() . 'hr_job_position')) { 
  $CI->db->query('ALTER TABLE `' . db_prefix() . "hr_job_position`
...
if (!$CI->db->table_exists(db_prefix() . 'hr_conselhos')) {
  $CI->db->query('CREATE TABLE `' . db_prefix() . "hr_conselhos` ( 
    `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
...
if (!$CI->db->field_exists('gerente_id' ,db_prefix() . 'hr_conselhos')) {
  $CI->db->query('ALTER TABLE `' . db_prefix() . "hr_conselhos`
    ADD COLUMN `gerente_id` INT(11) NULL;");
}

//adicionados

if (!$CI->db->table_exists(db_prefix() . 'hr_pelorios')) {
  $CI->db->query('CREATE TABLE `' . db_prefix() . "hr_pelorios` ( 
    `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
...
if (!$CI->db->field_exists('gerente_id' ,db_prefix() . 'hr_pelorios')) {
  $CI->db->query('ALTER TABLE `' . db_prefix() . "hr_pelorios`
    ADD COLUMN `gerente_id` INT(11) NULL;");
}

if (!$CI->db->table_exists(db_prefix() . 'hr_direcoes')) {
  $CI->db->query('CREATE TABLE `' . db_prefix() . "hr_direcoes` (
    `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
    `nome` varchar(200) NOT NULL,
    `email` varchar(600) NOT NULL,
    `pelorios_id` INT(11) NOT NULL,
    `created_at` datetime,
    `modified_at` datetime,
    `created_by`  int(11),
    PRIMARY KEY (`id`)
...
if (!$CI->db->field_exists('gerente_id' ,db_prefix() . 'hr_direcoes')) {
  $CI->db->query('ALTER TABLE `' . db_prefix() . "hr_direcoes`
    ADD COLUMN `gerente_id` INT(11) NULL;");
}
if (!$CI->db->table_exists(db_prefix() . 'hr_seccoes')) {
  $CI->db->query('CREATE TABLE `' . db_prefix() . "hr_seccoes` (
    `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
    `nome` varchar(200) NOT NULL,
    `email` varchar(600) NOT NULL,
...
if (!$CI->db->field_exists('gerente_id' ,db_prefix() . 'hr_seccoes')) {
  $CI->db->query('ALTER TABLE `' . db_prefix() . "hr_seccoes`
    ADD COLUMN `gerente_id` INT(11) NULL;");
}

if (!$CI->db->field_exists('direcoes_id' ,db_prefix() . 'departments')) { 
  $CI->db->query('ALTER TABLE `' . db_prefix() . "departments`
    ADD COLUMN `direcoes_id` INT(11) NOT NULL;");
}
if (!$CI->db->field_exists('gerente_id' ,db_prefix() . 'departments')) { 
  $CI->db->query('ALTER TABLE `' . db_prefix() . "departments`
    ADD COLUMN `gerente_id` INT(11) NULL;");
}
...
if (!$CI->db->field_exists('epf_no' ,db_prefix() . 'staff')) { 
  ...
  if (!$CI->db->field_exists('conselho_id' ,db_prefix() . 'staff')) { 
    $CI->db->query('ALTER TABLE `' . db_prefix() . "staff`
      ADD COLUMN `conselho_id` int(11) NULL,
      ADD COLUMN `pelorio_id` int(11) NULL,
      ADD COLUMN `direcao_id` int(11) NULL,
      ADD COLUMN `departamento_id` int(11) NULL,
      ADD COLUMN `seccao_id` int(11) NULL
    ;");
}