...
if($this->ci->input->post('direcao')) {
	$direcao = $this->ci->input->post('direcao');
	$where[] = 'AND ('.db_prefix().'hr_job_position.direcao_id = '.$direcao.')';
}
if($this->ci->input->post('departamento')) {
	$departamento = $this->ci->input->post('departamento');
	$where[] = 'AND ('.db_prefix().'hr_job_position.department_id = '.$departamento.')';
}
if($this->ci->input->post('seccao_id')) {
	$seccao_id = $this->ci->input->post('seccao_id');
	$where[] = 'AND ('.db_prefix().'hr_job_position.seccao_id = '.$seccao_id.')';
}


$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, ['position_id', 'direcao_id', 'department_id', 'seccao_id']);
...
foreach ($rResult as $aRow) {
	...
	if (has_permission('staffmanage_job_position', '', 'edit') || is_admin()) {
		$subjectOutput .= ' | <a href="#" class="editt_cargo" data-department="'.$aRow['department_id'].'" data-direcao="'.$aRow['direcao_id'].'" data-seccao="'.$aRow['seccao_id'].'" onclick="hrrecord_edit_job_position(this,' . $aRow['position_id'] . '); return false" data-name="' . $aRow['position_name'] . '" data-position_code="' . $aRow['position_code'] . '" data-department_id="' . $aRow['department_id'] . '" data-job_p_id="' . $aRow['job_p_id'] . '"  data-toggle="sidebar-right" >' . _l('hr_edit') . '</a>';
	}
	...
	if (new_strlen($aRow['job_position_description']) > 200) {
	...
	$row[] = get_hr_direcao_name($aRow['direcao_id']);


	// get department
	if ($aRow['department_id'] != null && $aRow['department_id'] != '') {
		$members = new_explode(',', $aRow['department_id']);
		$str = '';
		$j = 0;
	...
	$row[] = get_hr_seccao_name($aRow['seccao_id']);
