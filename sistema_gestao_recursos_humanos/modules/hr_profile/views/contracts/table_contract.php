...
if($this->ci->input->post('conselho')) {
	$conselho = $this->ci->input->post('conselho');
	$where[] = 'AND ('.db_prefix().'staff.conselho_id = '.$conselho.')';
}
if($this->ci->input->post('pelorio')) {
	$pelorio = $this->ci->input->post('pelorio');
	$where[] = 'AND ('.db_prefix().'staff.pelorio_id = '.$pelorio.')';
}
if($this->ci->input->post('direcao')) {
	$direcao = $this->ci->input->post('direcao');
	$where[] = 'AND ('.db_prefix().'staff.direcao_id = '.$direcao.')';
}
if($this->ci->input->post('departamento')) {
	$departamento = $this->ci->input->post('departamento');
	$where[] = 'AND ('.db_prefix().'staff.departamento_id = '.$departamento.')';
}
if($this->ci->input->post('seccao_id')) {
	$seccao_id = $this->ci->input->post('seccao_id');
	$where[] = 'AND ('.db_prefix().'staff.seccao_id = '.$seccao_id.')';
}
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix() . 'hr_staff_contract.id_contract', 'name_contracttype', 'firstname', 'conselho_id', 'pelorio_id', 'direcao_id', 'departamento_id', 'seccao_id', 'duration', 'unit', db_prefix() . 'hr_staff_contract.id_contract as id', 'lastname', 'signature']);
...
foreach ($rResult as $aRow) {
	...
	$row[] = get_hr_conselho_name($aRow['conselho_id']);
	$row[] = get_hr_pelorio_name($aRow['pelorio_id']);
