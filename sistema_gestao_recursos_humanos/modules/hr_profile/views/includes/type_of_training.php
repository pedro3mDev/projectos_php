...
<div>
	<div class="_buttons">
		<?php if(has_permission('hrm_setting', '', 'create') || is_admin() ){ ?>
			<a href="#" onclick="new_type_of_training(); return false;" class="btn btn-primary pull-left display-block">
	...
	<table class="table dt-table">
		...
		<tbody>
			<?php foreach($type_of_trainings as $type_of_training){ ?>
				<tr>
					<td><?php echo new_html_entity_decode($type_of_training['name']); ?></td>
					<td>

						<?php if(has_permission('hrm_setting', '', 'edit') || is_admin() ){ ?>
							<a href="#" onclick="edit_type_of_training(this,<?php echo new_html_entity_decode($type_of_training['id']); ?>); return false" data-name="<?php echo new_html_entity_decode($type_of_training['name']); ?>" class="btn btn-default btn-icon"><i class="fa-regular fa-pen-to-square"></i></a>
						<?php } ?>
