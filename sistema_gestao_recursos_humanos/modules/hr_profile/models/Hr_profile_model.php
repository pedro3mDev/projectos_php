...
class Hr_profile_model extends App_Model
{
	...
	public function get_data_chart()
	{
		...
		$dep_tree = [];
		$conselhos = $this->db->get(db_prefix() . 'hr_conselhos')->result_array();
		foreach ($conselhos as $conselho) {
			$total = $this->db->get_where('staff', ['conselho_id' => $conselho['id']])->num_rows();
			array_push($dep_tree, [
				'id' => $conselho['id'],
				'name' => $conselho['nome'],
				'title'    => get_staff_full_name($conselho['gerente_id'] ?? 0),
				'image' => staff_profile_image($conselho['gerente_id'] ?? 0, [
					'staff-profile-image-small staff-chart-padding',
				]),
				'children' => $this->get_child_pelorio_staff_chart($conselho['id']),
				'reality_now' => _l('hr_current_personnel') . ': '. $total,
				'dp_user_icon' => '"fa fa-user-o"',
				// 'job_position' => '<a href="javascript:;" data-id="'.$conselho['id'].'" data-conselho="'.$conselho['nome'].'" data-responsavel="'.$conselho['gerente_id'] .'" class="btn_edit_conselho">Editar</a>',
				'job_position' => 'Conselho Cargo',
			]);
		}
		return $dep_tree;
	}

	private function get_child_pelorio_staff_chart($id)
	{
		$pelorios = $this->db->get_where(db_prefix() . 'hr_pelorios', ['conselho_id', $id])->result_array();
		$dep_tree = [];
		foreach ($pelorios as $dep) {
			$total = $this->db->get_where('staff', ['pelorio_id' => $dep['id']])->num_rows();
			array_push($dep_tree, [
				'id' => $dep['id'],
				'name' => $dep['nome'],
				'title'    => get_staff_full_name($dep['gerente_id'] ?? 0),
				'image' => staff_profile_image($dep['gerente_id'] ?? 0, [
					'staff-profile-image-small staff-chart-padding',
				]),
				'children' => $this->get_child_direcao_staff_chart($dep['id']),
				'reality_now' => _l('hr_current_personnel') . ': '. $total,
				'dp_user_icon' => '"fa fa-user-o"',
				'job_position' => 'Pelorio Cargo',
			]);
		}
		return $dep_tree;
	}
	private function get_child_direcao_staff_chart($id)
	{
		$direcao = $this->db->get_where(db_prefix() . 'hr_direcoes', ['pelorio_id', $id])->result_array();
		$dep_tree = [];
		foreach ($direcao as $dep) {
			$total = $this->db->get_where('staff', ['direcao_id' => $dep['id']])->num_rows();
			array_push($dep_tree, [
				'id' => $dep['id'],
				'name' => $dep['nome'],
				'title'    => get_staff_full_name($dep['gerente_id'] ?? 0),
				...
				'children' => $this->get_child_departamento_staff_chart($dep['id']),
				'reality_now' => _l('hr_current_personnel') . ': '. $total,
				'dp_user_icon' => '"fa fa-user-o"',
				'job_position' => 'Direção Cargo',
			]);
		}
		return $dep_tree;
	}
	private function get_child_departamento_staff_chart($id)
	{
		$departments = $this->db->get_where(db_prefix() . 'departments', ['direcoes_id', $id])->result_array();
		$dep_tree = [];
		foreach ($departments as $dep) {
			$total = $this->db->get_where('staff', ['departamento_id' => $dep['departmentid']])->num_rows();
			array_push($dep_tree, [
				'id' => $dep['departmentid'],
				'name' => $dep['name'],
				'title'    => get_staff_full_name($dep['gerente_id'] ?? 0),
				...
				'children' => $this->get_child_seccao_staff_chart($dep['departmentid']),
				'reality_now' => _l('hr_current_personnel') . ': '. $total,
				'dp_user_icon' => '"fa fa-user-o"',
				'job_position' => 'Departamento Cargo',
			]);
		}
		return $dep_tree;
	}
	private function get_child_seccao_staff_chart($id)
	{
		$seccoes = $this->db->get_where(db_prefix() . 'hr_seccoes', ['departamento_id', $id])->result_array();
		$dep_tree = [];
		foreach ($seccoes as $dep) {
			$total = $this->db->get_where('staff', ['seccao_id' => $dep['id']])->num_rows();
			array_push($dep_tree, [
				'id' => $dep['id'],
				'name' => $dep['nome'],
				'title'    => get_staff_full_name($dep['gerente_id'] ?? 0),
	...
	public function total_onbording()
	{
		$query = $this->db->get('hr_rec_transfer_records')->num_rows();
		return $query;
	}
	public function dashboard_onbording()
	{
		$this->db->select('tblhr_rec_transfer_records.*, staff.staffid, staff.firstname, staff.lastname');
		$this->db->from('tblhr_rec_transfer_records');
		$this->db->join('staff', 'staff.staffid = tblhr_rec_transfer_records.staffid', 'left');
		$query = $this->db->get()->result_array();
		return $query;
	}
	public function total_treinamentos()
	{
		$query = $this->db->get('hr_jp_interview_training')->num_rows();
		return $query;
	}
	public function total_contractos()
	{
		$query = $this->db->get('hr_staff_contract')->num_rows();
		return $query;
	}
	public function total_staff_homem()
	{
		$query = $this->db->get_where('staff', ['sex' => 'male'])->num_rows();
		return $query;
	}
	public function total_staff_mulher()
	{
		$query = $this->db->get_where('staff', ['sex' => 'female'])->num_rows();
		return $query;
	}
	public function total_staff_indefinido()
	{
		$query = $this->db->get_where('staff', ['sex' => null])->num_rows();
		return $query;
	}
	public function dasboard_func_genero()
	{
		$homens = "'" . $this->total_staff_homem() . "',";
		$mulheres = "'" . $this->total_staff_mulher() . "',";
		$nao_definidos = "'" . $this->total_staff_indefinido() . "'";
		return "[$homens $mulheres $nao_definidos]";
	}
	...
	public function total_status_contracts($status = "valid")
	{
		$this->db->where('contract_status', $status);
		$query = $this->db->get(db_prefix() . 'hr_staff_contract');
		return $query->num_rows();
	}
	public function total_trainings_per_month($year): array
	{
		// Criando um array com todos os meses do ano
		$months = [
			1 => 'Janeiro',
			2 => 'Fevereiro',
			3 => 'Março',
		...
		// Consultando os treinamentos do ano atual, agrupados por mês
		$this->db->select('MONTH(datecreated) as month, COUNT(*) as total');
		$this->db->where('YEAR(datecreated)', $year);
		$this->db->group_by('MONTH(datecreated)');
		$this->db->order_by('MONTH(datecreated)', 'ASC');
		$query = $this->db->get(db_prefix() . 'hr_position_training');
		$results = $query->result_array();

		// Inicializando um array para armazenar o total de treinamentos por mês
		$monthly_data = [];

		// Preenche todos os meses com 0 inicialmente
		foreach ($months as $month_num => $month_name) {
			$monthly_data[$month_num] = [
				'month' => $month_name,
				'total' => 0 // Inicializa com 0
			];
		}

		// Atualizando os meses que possuem registros com os valores da consulta
		foreach ($results as $row) {
			$monthly_data[$row['month']]['total'] = $row['total'];
		}

		// Retorna os dados do mês com o total de treinamentos, incluindo os meses com 0
		return $monthly_data;
	}


	/**
	 * Busca pelórios por ID do conselho.
	 *
	 * @param int $conselho_id ID do conselho para filtro.
	 * @return array Lista de pelórios associados ao conselho.
	 */
	public function get_pelorios_by_conselho($conselho_id)
	{
		$this->db->where('conselho_id', $conselho_id);
		$query = $this->db->get(db_prefix() . 'hr_pelorios');
		return $query->result_array();
	}
	...
	/**
	 * Busca direções por ID do pelório.
	 *
	 * @param int $pelorios_id ID do pelório para filtro.
	 * @return array Lista de direções associadas ao pelório.
	 */
	public function get_direcoes_by_pelorio($pelorios_id)
	{
		$this->db->where('pelorios_id', $pelorios_id);
		$query = $this->db->get(db_prefix() . 'hr_direcoes');
		return $query->result_array();
	}
	/**
	 * Busca departamentos por ID da direção.
	 *
	 * @param int $direcoes_id ID da direção para filtro.
	 * @return array Lista de departamentos associados à direção.
	 */
	public function get_departments_by_direcao($direcoes_id)
	{
		$this->db->where('direcoes_id', $direcoes_id);
		$query = $this->db->get(db_prefix() . 'departments');
		return $query->result_array();
	}
	/**
	 * Busca seções por ID do departamento.
	 *
	 * @param int $departments_id ID do departamento para filtro.
	 * @return array Lista de seções associadas ao departamento.
	 */
	public function get_seccoes_by_departamento($departments_id)
	{
		$this->db->where('departments_id', $departments_id);
		$query = $this->db->get(db_prefix() . 'hr_seccoes');
		return $query->result_array();
	}
