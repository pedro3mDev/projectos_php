...
/*
Module Name: Gestão de Integração
Description:  Gestão de Integração
...
function hr_profile_module_init_menu_items() {
    ...
    // Verifica se o usuário tem permissão para ver o módulo de integração
    if (
        has_permission('hrm_dashboard', '', 'view') ||
        has_permission('staffmanage_orgchart', '', 'view') ||
        has_permission('hrm_reception_staff', '', 'view') ||
        has_permission('hrm_hr_records', '', 'view') ||
        has_permission('staffmanage_job_position', '', 'view') ||
        has_permission('staffmanage_training', '', 'view') ||
        has_permission('hr_manage_q_a', '', 'view') ||
        has_permission('hrm_contract', '', 'view') ||
        has_permission('hrm_dependent_person', '', 'view') ||
        has_permission('hrm_procedures_for_quitting_work', '', 'view') ||
        has_permission('hrm_report', '', 'view') ||
        has_permission('hrm_setting', '', 'view')
    ) {
        // Obtém a rota atual
        $current_class = $CI->router->fetch_class();
        $current_method = $CI->router->fetch_method();

        // Define as rotas que correspondem ao módulo de integração
        $hr_profile_routes = [
            'hr_profile',
            'dashboard',
            'reception_staff',
        ...
        // Verifica se a rota atual pertence ao módulo de integração, mas não a página inicial
        if (in_array($current_class, $hr_profile_routes) && $current_class != 'dashboard') {
            $CI->app_menu->add_sidebar_menu_item('hr_profile', [
                'name' => _l('Gestão Integração'),
                'icon' => 'fa fa-users',
                'position' => 3,
            ]);
...
function hr_profile_permissions()
{
	...
	//Onboarding Process
	register_staff_capabilities('hrm_reception_staff', $capabilities_3, _l('HR_reception_staff_c'));
	//Hr Profile
	register_staff_capabilities('hrm_hr_records', $capabilities_3, _l('hr_hr_records'));
	//Job Description
	register_staff_capabilities('staffmanage_job_position', $capabilities_3, _l('HR_job_escription_c'));
	//Training
	register_staff_capabilities('staffmanage_training', $capabilities_3, _l('hr_training_c'));
	//Q&A
	register_staff_capabilities('hr_manage_q_a', $capabilities_2, _l('HR_q&a'));
	//Contracts
	register_staff_capabilities('hrm_contract', $capabilities_3, _l('HR_contract_c'));
	//Dependent Persons
	...
	//Settings
	register_staff_capabilities('hrm_setting', $capabilities, _l('HR_setting_c'));
