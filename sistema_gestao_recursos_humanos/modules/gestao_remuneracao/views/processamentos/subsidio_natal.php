        ...
        <div class="row">
            <div class="col-md-10">
                <a class="dashboard-link" href="" style="font-size: 16px; color: #333;">
                    Gestão de Remuneração / Processamentos / Subsídio de Natal
                </a>
            </div>
                ...
                <div class="panel_s">
                    <div class="panel-body">
                        ...
                        <table class="table table-hover table-bordered table-responsive dt-table">
                            ...
                            <tbody>
                                <?php foreach ($subsidio_natal as $subsidio): ?>
                                    <tr>
                                        <td> <a
                                                href="#"><?= htmlspecialchars($subsidio['primeiro_nome'] . " " . $subsidio['segundo_nome']); ?></a>
                                        </td>a
                                        <td> <a href="#"><?= htmlspecialchars($subsidio['valor']); ?></a>
                                        </td>
                                        <td> <a href="#"><?= date('d/m/Y', strtotime($subsidio['data_pagamento'])); ?></a>
                                        </td>
                                        <td>
                                            <a href="<?php echo admin_url('gestao_remuneracao/processamentos_visualizar_subsidio_natal/'. $subsidio['id']) ?>"
                                                class="btn btn-success btn-icon">
                                                <i style="color: white;" class="fa fa-eye"></i>
                                            </a>
                                            <a href="#" class="btn btn-default btn-edit-subsidio_natal"
                                                data-id="<?= $subsidio['id']; ?>" data-valor="<?= $subsidio['valor']; ?>"
                                                data-data_pagamento="<?= $subsidio['data_pagamento']; ?>"
                                                data-staff_id="<?= $subsidio['staff_id']; ?>" data-toggle="modal"
                                                data-target="#editar">
                                                <i class="fa fa-edit"></i>
                                            </a>

                                            <a onclick="return confirm('Tem certeza que deseja excluir?');"
                                                href="<?= base_url('gestao_remuneracao/delete_subsidio_natal/' . $subsidio['id']); ?>"
...
<div class="modal" id="modal_add" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-md">
        <?php echo form_open(admin_url('gestao_remuneracao/add_subsidio_natal'), array('method' => 'post')); ?>
...
<?= form_open(admin_url('gestao_remuneracao/editar_subsidio_natal'), array('method' => 'post', 'id' => 'form_edit_subsidio_natal')) ?>
...
<script>
    ...
    $(document).ready(function () {
        $(document).on('click', '.btn-edit-subsidio_natal', function () {
            let id = $(this).data('id');
