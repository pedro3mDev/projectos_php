...
class Pedidos extends AdminController {
	...
	public function editar_pedido ($id) {
        ...
        if ($data['destino'] == 'Indefinido') {
            set_alert('danger', 'Actualize os dados para planejamento de Viagem');
            redirect('gestao_viagens/pedidos/');
        }
    ...
    public function status_pedido_aprovar($id) {
        ...
        if ($vf['destino'] == 'Indefinido') {
            set_alert('danger', 'Actualize os dados para planejamento de Viagem');
			return redirect('gestao_viagens/pedidos/pedido/'.$id);
        }

        if ($vf['tipo_viagem_id'] == 1) {
            $de     = new DateTime($vf['data_inicio']);
            $ate    = new DateTime($vf['data_fim']);
            $intervalo = $de->diff($ate);

            $staffs = $this->Gv_pedido_model->get_staff($id);
            foreach ($staffs as $s) {
                $total_dias = $this->Gv_gestao_viagem_model->verificar_ferias($s['staff_id']) + $intervalo->days;
                if (get_conf_gestao_ferias()->limite_dias_ferias <= $total_dias) {
                    $message =  _l('Não é possivel aprovar, o '.$s['firstname'] .' '. $s['lastname'] .' Atingiste o limite máximo de dias de férias!');
                    set_alert('warning', $message);
                    return redirect('gestao_viagens/pedidos/pedido/'.$id);
                }
                $vf_feiras_existe = $this->Gv_pedido_model->vf_ferias_existe($s['staff_id'], $vf['data_inicio'], $vf['data_fim']);

                if ($vf_feiras_existe) {
                    $message =  _l('A data selecionada já esta associada a uma féria de ' . $s['firstname'] .' '. $s['lastname']);
                    set_alert('warning',$message);
                    return redirect('gestao_viagens/pedidos/pedido/'.$id);
                }
            }
            $objetivo = $vf['objetivo'];
            $de = $vf['data_inicio'];
            $ate = $vf['data_fim'];
            foreach ($staffs as $s) {
                $dados = [
                ...
                $this->Gv_pedido_model->create_ferias($dados);
            }
        }
    ...
    public function reservas() {
        ...
        $data['total_reserva'] = $this->Gv_gestao_viagem_model->total_reserva();
		$data['total_reserva_penedntes'] = $this->Gv_gestao_viagem_model->total_reserva(1);
		$data['total_reserva_rejeitados'] = $this->Gv_gestao_viagem_model->total_reserva(3);
		$data['total_reserva_aprovados'] = $this->Gv_gestao_viagem_model->total_reserva(2);
        ...
        $data['hotel_f'] = $this->Gv_gestao_viagem_model->get_hotel(2);
        ...
        if (!$this->form_validation->run()) {
			...
			if (!empty($_FILES['arquivo']['name'])) {
				...
				if (!$this->upload->do_upload('arquivo')) {
					...
					return redirect('gestao_viagens/pedidos/reservas/?group=reservas');
				}
				else {
					$file_upload = TRUE;
            ...
            $data = [
                ...
                'aprovadores' => json_encode($this->input->post('aprovadores[]')) ?? '[]',
                'arquivo_visto' => $file_upload ? $upload_data['file_name'] : null,
            ];
        ...
        if (!$this->form_validation->run()) {
            ...
            if (!empty($_FILES['arquivo']['name'])) {
				...
				if (!$this->upload->do_upload('arquivo')) {
					...
					$upload_data = $this->upload->data();
					$data['arquivo_visto'] = $upload_data['file_name'];
				}
			}
