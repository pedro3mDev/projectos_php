...
function rec_motivo_exists($value){
    $CI = & get_instance();
    $i = count($CI->db->query('Select * from '.db_prefix()."rec_motivo where motivo = '{$value}' ")->result_array());
    if($i == 0){
        return 0;
    }
    if($i > 0){
...
function get_rec_motivo_name($id)
{
    $CI = & get_instance();
    $CI->db->where('id',$id);
    $rec_motivo = $CI->db->get(db_prefix().'rec_motivo')->row();

    if($rec_motivo){
        return $rec_motivo->motivo;
    }else{
        return '';
    }

}
function rec_tipo_sanguineo_exists($value){
    $CI = & get_instance();
    $i = count($CI->db->query('Select * from '.db_prefix()."rec_tipo_sanguineo where tipo_sanguineo = '{$value}' ")->result_array());
    if($i == 0){
        return 0;
    }
    if($i > 0){
...
function get_tipo_sanguineo_name($id)
{
    $CI = & get_instance();
    $CI->db->where('id',$id);
    $rec_tipo_sanguineo = $CI->db->get(db_prefix().'rec_tipo_sanguineo')->row();

    if($rec_tipo_sanguineo){
        return $rec_tipo_sanguineo->tipo_sanguineo;
    }else{
        return '';
    }
...
function send_email_recrutamento($to='',$subject='',$message=''){
    $CI = &get_instance();
...
function get_rec_direcao_name($id){
    $CI           = & get_instance();
    if($id != 0){
        $CI->db->where('id',$id);
        $dpm = $CI->db->get(db_prefix().'hr_direcoes')->row();
        if($dpm->nome){
            return $dpm->nome;
        }else{
            return '';
...
function get_rec_seccao_name($id){
    $CI           = & get_instance();
    if($id != 0){
        $CI->db->where('id',$id);
        $dpm = $CI->db->get(db_prefix().'hr_seccoes')->row();
        if($dpm->nome){
            return $dpm->nome;
        }else{
            return '';
...
//Algoritimo de algoritmo de matching
function alg_matching($id_campaign='',$id_cadidate=''){
    $CI           = & get_instance();
    $porcentagem  = 0;
    $habilidades_tt = 0;
    $habilidades_comp = 0;

    $CI->db->where('id',$id_cadidate);
    $data = $CI->db->get(db_prefix().'rec_candidate')->result_array();
    
    if (isset(get_rec_position($id_campaign)->job_skill)) {
        $habilidades = get_rec_position($id_campaign)->job_skill;
    }else {
        $habilidades = '';
    }
  
    if (isset($data[0]['skill'])) {
       $emp_skill = $data[0]['skill'];
    }else {
        $emp_skill = '';
    }

    $habilidades_tt = count(explode(',',$habilidades ));
    $habilidades_comp = compararHabilidades($emp_skill,$habilidades);
    $porcentagem = number_format(($habilidades_comp / $habilidades_tt) * 100,0);

    $html = '';
    if ($porcentagem >= 0 && $porcentagem <= 30) {
        $html = '<span class="text-danger"> ('.$porcentagem .' %) </span>';
    }elseif ($porcentagem >= 31 && $porcentagem <= 49) {
        $html = '<span class="text-warning"> ('.$porcentagem .' %) </span>';
    }elseif ($porcentagem >= 50 && $porcentagem <= 70) {
        $html = '<span class="text-primary"> ('.$porcentagem .' %) </span>';
    }elseif ($porcentagem >= 71 && $porcentagem <= 100) {
        $html = '<span class="text-success"> ('.$porcentagem .' %) </span>';
    }else {
        $html = '<span class="text-danger"> ('.$porcentagem .' %) </span>';
    }
    return $html;
...
// pega pega o numero de candidatos por campanha
function get_candidate_by_capmapign($id=''){
    $CI           = & get_instance();
    $CI->db->where('rec_campaign',$id);
    $datas = $CI->db->get(db_prefix().'rec_candidate')->result_array();
    return count($datas);
}


// pega pega o numero de campanha por dpt
function get_capmapign_by_dpt($id=''){
    $CI           = & get_instance();
    $CI->db->where('cp_department',$id);
    $CI->db->where('cp_status',3);
    $datas = $CI->db->get(db_prefix().'rec_campaign')->result_array();
    return count($datas);
}
...
function get_skill_name($id){
    ...
    if (is_numeric($id)) {
        $CI->db->where('id', $id);
        return $CI->db->get(db_prefix() . 'rec_skill')->row();
    }


}
function get_skill_name_byId($id = ''){
    $CI           = & get_instance();

    $name;
    $CI->db->where('id', $id);
    $name = $CI->db->get(db_prefix() . 'rec_skill')->row();
    return  isset($name->skill_name)?$name->skill_name:'none';
}
function get_rec_position($id=''){
    $CI           = & get_instance();
    if (is_numeric($id)) {
         $CI->db->where('id', $id);
         return $CI->db->query("SELECT po.position_name as position_name,po.job_skill as job_skill FROM tblrec_campaign as cp JOIN tblrec_job_position as po ON cp.cp_position = po.position_id WHERE cp.cp_id= '$id' ")->row();
    }

    if ($id == false) {
        return $CI->db->query("SELECT po.position_name as position_name,po.job_skill as job_skill FROM tblrec_campaign as cp JOIN tblrec_job_position as po ON cp.cp_id = po.position_id")->result_array();
    }

}


function compararHabilidades($habilidades_funcionario, $habilidades_vaga) {
    $habilidades_comuns = array_intersect(explode(',',$habilidades_funcionario), explode(',',$habilidades_vaga));
    return count($habilidades_comuns);
}
