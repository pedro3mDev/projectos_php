...
if (!$CI->db->field_exists('direcao_id' ,db_prefix() . 'rec_proposal')) { 
  $CI->db->query('ALTER TABLE `' . db_prefix() . "rec_proposal`
    ADD COLUMN `direcao_id` int(11) NULL
  ;");
}
if (!$CI->db->field_exists('seccao_id' ,db_prefix() . 'rec_proposal')) { 
  $CI->db->query('ALTER TABLE `' . db_prefix() . "rec_proposal`
    ADD COLUMN `seccao_id` int(11) NULL
  ;");
}
if (!$CI->db->field_exists('cp_motivo' ,db_prefix() . 'rec_proposal')) { 
  $CI->db->query('ALTER TABLE `' . db_prefix() . "rec_proposal`
    ADD COLUMN `cp_motivo` int(11) NOT NULL
  ;");
}
if (!$CI->db->field_exists('cp_tipo_sanguineo' ,db_prefix() . 'rec_proposal')) { 
  $CI->db->query('ALTER TABLE `' . db_prefix() . "rec_proposal`
    ADD COLUMN `cp_tipo_sanguineo` int(11) NULL
  ;");
}
...
if (!$CI->db->field_exists('cp_motivo' ,db_prefix() . 'rec_campaign')) { 
  $CI->db->query('ALTER TABLE `' . db_prefix() . "rec_campaign`
    ADD COLUMN `cp_motivo` int(11) NOT NULL
  ;");
}
if (!$CI->db->field_exists('cp_tipo_sanguineo' ,db_prefix() . 'rec_campaign')) { 
  $CI->db->query('ALTER TABLE `' . db_prefix() . "rec_campaign`
    ADD COLUMN `cp_tipo_sanguineo` int(11) NULL
  ;");
}
...
if (!$CI->db->field_exists('last_ip' ,db_prefix() . 'rec_candidate')) { 
  $CI->db->query('ALTER TABLE `' . db_prefix() . "rec_candidate`
    ADD COLUMN `last_ip` varchar(50) NOT NULL AFTER `status`
  ;");
}
if (!$CI->db->field_exists('last_login' ,db_prefix() . 'rec_candidate')) { 
  $CI->db->query('ALTER TABLE `' . db_prefix() . "rec_candidate`
...
if (!$CI->db->table_exists(db_prefix() . 'rec_skill')) {
  ...
  if (!$CI->db->field_exists('direcao_id' ,db_prefix() . 'rec_campaign')) { 
    $CI->db->query('ALTER TABLE `' . db_prefix() . "rec_campaign`
      ADD COLUMN `direcao_id` int(11) NULL
    ;");
  }
  if (!$CI->db->field_exists('seccao_id' ,db_prefix() . 'rec_campaign')) { 
    $CI->db->query('ALTER TABLE `' . db_prefix() . "rec_campaign`
      ADD COLUMN `seccao_id` int(11) NULL
    ;");
  }
...
if (recruitment_row_options_exist('"recruitment_create_campaign_with_plan "') == 0){
  ...
  if (!$CI->db->table_exists(db_prefix() . 'rec_motivo')) {
    $CI->db->query('CREATE TABLE `' . db_prefix() . "rec_motivo` (
      `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
...
if (rec_motivo_exists('Contratação por Subistituição') == 0) {
  $CI->db->query('INSERT INTO `'.db_prefix().'rec_motivo` (`motivo`) VALUES ("Contratação por Subistituição");');
}
if (!$CI->db->table_exists(db_prefix() . 'rec_tipo_sanguineo')) {
  $CI->db->query('CREATE TABLE `' . db_prefix() . "rec_tipo_sanguineo` (
    `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
    `tipo_sanguineo` varchar(200) NOT NULL,
    `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
    `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
    PRIMARY KEY (`id`)
  ) ENGINE=InnoDB DEFAULT CHARSET=" . $CI->db->char_set . ';');
}
if (rec_tipo_sanguineo_exists('A+') == 0) {
  $CI->db->query('INSERT INTO `'.db_prefix().'rec_tipo_sanguineo` (`tipo_sanguineo`) VALUES ("A+");');
}
if (rec_tipo_sanguineo_exists('A-') == 0) {
  $CI->db->query('INSERT INTO `'.db_prefix().'rec_tipo_sanguineo` (`tipo_sanguineo`) VALUES ("A-");');
}
if (rec_tipo_sanguineo_exists('B+') == 0) {
  $CI->db->query('INSERT INTO `'.db_prefix().'rec_tipo_sanguineo` (`tipo_sanguineo`) VALUES ("B+");');
}
if (rec_tipo_sanguineo_exists('B-') == 0) {
  $CI->db->query('INSERT INTO `'.db_prefix().'rec_tipo_sanguineo` (`tipo_sanguineo`) VALUES ("B-");');
}
if (rec_tipo_sanguineo_exists('AB+') == 0) {
  $CI->db->query('INSERT INTO `'.db_prefix().'rec_tipo_sanguineo` (`tipo_sanguineo`) VALUES ("AB+");');
}
if (rec_tipo_sanguineo_exists('AB-') == 0) {
  $CI->db->query('INSERT INTO `'.db_prefix().'rec_tipo_sanguineo` (`tipo_sanguineo`) VALUES ("AB-");');
}
if (rec_tipo_sanguineo_exists('O+') == 0) {
  $CI->db->query('INSERT INTO `'.db_prefix().'rec_tipo_sanguineo` (`tipo_sanguineo`) VALUES ("O+");');
}
if (rec_tipo_sanguineo_exists('O-') == 0) {
  $CI->db->query('INSERT INTO `'.db_prefix().'rec_tipo_sanguineo` (`tipo_sanguineo`) VALUES ("O-");');
}