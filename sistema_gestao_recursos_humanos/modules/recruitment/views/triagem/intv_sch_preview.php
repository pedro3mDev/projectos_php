...
?></td>
          ...
          <?php foreach ($intv_sch->list_candidate as $cd) {
            $porcentagem = alg_matching($intv_sch->campaign, $cd['id']);
              ...
              <div class="row">
                  <div class="thumbnail">
                    <div class="caption" onclick="location.href='<?php echo admin_url('recruitment/candidate/' . $cd['candidate']) ?>'">
                      ...
                      <h4 id="thumbnail-label"><?php echo candidate_profile_image($cd['candidate'], ['staff-profile-image-small mright5'], 'small') . ' #' . $cd['candidate_code'] . ' - ' . $cd['candidate_name'] . ' ' . $cd['last_name']; ?> <?php echo $porcentagem; ?>  </span>  </h4>
