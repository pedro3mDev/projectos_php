...
{   
    ...
    public function __construct() {
        ...
        // Forçar a Remoção do Admin
        $url_completa = $this->uri->segment_array();
        if(!empty($url_completa) && $url_completa[1] === 'admin') {
            array_shift($url_completa);
            $nova_url = base_url(implode('/', $url_completa));
            redirect($nova_url);
        }
    }
    ...
    public function minhas_candidaturas()
    {
        if (!is_candidate_logged_in()) {
            redirect(site_url('recruitment/authentication_candidate/login'));
        }
        $data['title'] = _l('Minha Candidatura');
        $data['candidaturas'] = $this->recruitment_model->get_candidaturas();

        $this->data($data);
        $this->view('recruitment_portal/minhas_candidaturas');
        $this->layout();
    }
    ...
    public function perfil()
    {
        ...
        $data['title'] = _l('Minha Candidatura');
        $data['perfil'] = $this->recruitment_model->first_candiadato();

        $this->data($data);
        $this->view('recruitment_portal/perfil');
        $this->layout();
    }
