...
class recruitment extends AdminController
{
	...
	public function setting()
	{
		...
		$data['tipo_sanguineo'] = $this->recruitment_model->get_tipo_sanguineo();

		$data['motivo'] = $this->recruitment_model->get_motivo();
	...
	public function add_motivo () {
		...
		if (!$this->form_validation->run()) {
			...
			$insert = $this->recruitment_model->create_motivo($data);
			set_alert('success',"Cadastrado com sucesso");
			redirect('recruitment/setting?group=motivos');
		}
	}
	public function editar_motivo ($id) {
		$this->http_method('POST');
		$this->recruitment_model->first_motivo($id);

		$data = [
            'motivo' => $this->input->post('e_motivo') ?? '',
            'descricao' => $this->input->post('e_descricao') ?? '',
        ];
		$this->form_validation->set_rules('e_motivo', _l('motivo'), 'required', ['required' => 'Preencha o campo {field}']);
		if (!$this->form_validation->run()) {
			set_alert('danger',"Preenchas os campos correctamente");
			redirect('recruitment/setting?group=motivos');
        }
		else {
			$this->recruitment_model->update_motivo($data, $id);
	...
	public function delete_motivo ($id) {
		$this->recruitment_model->first_motivo($id);
		$this->recruitment_model->delete_motivo($id);
	...
	public function job_position()
	{
		if ($this->input->post()) {
			...
			$this->form_validation->set_rules('position_name', 'cargo', 'required', ['required' => 'Preencha o campo {field}']);
			$this->form_validation->set_rules('job_skill[]', 'Nome da habilidade', 'required', ['required' => 'Preencha o campo {field}']);
			$this->form_validation->set_rules('industry_id', 'Indústria', 'required', ['required' => 'Preencha o campo {field}']);
			if (!$this->form_validation->run()) {
				set_alert('danger', 'Preencha todos os campos obrigatórios');
				return redirect(admin_url('recruitment/setting?group=job_position'));
			}
	...
	public function recruitment_proposal($id = '')
	{
		...
		$data['total_plano_campanha'] = $this->recruitment_model->plano_campanha();
		$data['total_plano_campanha_plano'] = $this->recruitment_model->plano_campanha(1);
		$data['total_plano_campanha_aprovadas'] = $this->recruitment_model->plano_campanha(2);
		$data['total_plano_campanha_rejeitadas'] = $this->recruitment_model->plano_campanha(4);
	...
	public function recruitment_campaign($id = '')
	{
		...
		$data['direcoes'] = $this->db->get(db_prefix() . 'hr_direcoes')->result_array();
		// $data['depertamentos'] = $this->departments_model->get();
	...
	public function candidate_profile()
	{
		...
		$data['total_candidaturas'] = $this->recruitment_model->total_candidaturas();
		$data['total_candidaturas_1'] = $this->recruitment_model->total_candidaturas(1);
		$data['total_candidaturas_3'] = $this->recruitment_model->total_candidaturas(3);
		$data['total_candidaturas_4'] = $this->recruitment_model->total_candidaturas(4);
	...
	public function triagem($id = '')
	{
		...
		$data['staffs'] = $this->staff_model->get();
		$data['candidates'] = $this->tbl_triagem($this->recruitment_model->get_candidates_triagem($id), $id);
		$data['list_cd'] = $this->recruitment_model->get_list_cd();
		$data['rec_campaigns'] = $this->recruitment_model->get_rec_campaign_triagem();
		$data['positions'] = $this->recruitment_model->get_job_position();
		$data['interview_id'] = $id;
		$data['title'] = _l('triagem');
		$data['rec_campaigns_header'] = '';
		if ($id != '') {
			$data['rec_campaigns_header'] = $this->header_triagem($this->recruitment_model->get_rec_campaign_triagem($id));
		}
		$this->load->view('triagem/triagem', $data);
	}


	public function table_triagem()
	{
		if ($this->input->is_ajax_request()) {
			$this->app->get_table_data(module_views_path('recruitment', 'triagem/table_triagem'));
		}
	}

	public function tbl_triagem($data = [], $id_campaign = '')
	{

		$html = '';
		if (count($data) > 0) {

			$html .= '<table id="" class="table dt-table">';
			$html .= '<thead>';
			$html .= ' <tr>';
			$html .= '    <th>Nome</th>';
			$html .= '    <th>Habilidades</th>';
			$html .= '    <th width="30%">Porcentagem</th>';
			$html .= ' </tr>';
			$html .= '</thead>';
			$html .= '<tbody>';
			...
			foreach ($data as $key => $value) {
				$skill = [];
				if (isset($value['skill']) && isset($value['skill']) != '') {
					$skill = explode(',', $value['skill']);
				}
				$html .= '<tr>';
				$html .= '  <td>' . $value['candidate_name'] . '</td>';
				$html .= '  <td>';
				foreach ($skill as $val) {
					$html .=  get_skill_name_byId($val) . '<br>';
				}
				$html .= '  </td>';
				$html .= '  <td>' . alg_matching($id_campaign, $value['id']) . '</td>';
				$html .= '</tr>';
			}
			$html .= '</tbody>';
			$html .= '</table>';
			...
			$html = ' <span class="text-ranger">Sem dados, por faor, seleciona uma campanha que tenha mais dados!</span>';
		}
		return $html;
	}
	public function header_triagem($data = [])
	{
		$skill = [];

		if (isset($data->cp_position)) {
			$this->db->where('position_id', $data->cp_position);
			$job_skill = $this->db->get(db_prefix() . 'rec_job_position')->row()->job_skill;
			$skill = explode(',', $job_skill);
		}

		$html = '';
		$html .= '<table class="table table-xl" style="width: 40%;">';
		$html .= '<tbody>';
		$html .= '	<tr>';
		$html .= '	  <td> <strong>CAMPANHA</strong></td>';
		$html .= '	  <td>' . $data->campaign_name . '</td>';
		$html .= '	</tr>';
		$html .= '	<tr>';
		$html .= '	  <td> <strong>HABILIDADES</strong></td>';
		$html .= '  <td>';
		foreach ($skill as $val) {
			$html .=  get_skill_name_byId($val) . '<br>';
		}
	...
	public function triagem_geral($id = '')
	{

		$data['departments'] = $this->departments_model->get_all();
		$data['rec_campaigns'] = $this->tbl_campaigns($this->recruitment_model->get_rec_campaign_triagem_by_dpt($id));
		$data['list_cd'] = $this->recruitment_model->get_list_cd();
		$data['positions'] = $this->recruitment_model->get_job_position();
		$data['staffs'] = $this->staff_model->get();
		$data['campaign_id'] = $id;
		$data['rec_channel_form']	= $this->recruitment_model->get_recruitment_channel();
		$data['company_list'] = $this->recruitment_model->get_company();
		//var_dump($data['rec_campaigns']);die;  
		$data['interview_id'] = $id;
		$data['title'] = _l('Listar campanhas');
		$this->load->view('triagem/triagem_geral', $data);
	}

	public function tbl_campaigns($data = [])
	{
		...
		if (count($data) > 0) {
			...
			$html .= '    <th>N Candidatos</th>';
			$html .= '    <th width="10%">Opções</th>';
			$html .= ' </tr>';
			$html .= '</thead>';
			$html .= '<tbody>';
			...
			foreach ($data as $key => $value) {
				$html .= '<tr>';
				$html .= '  <td>' . $value['campaign_code'] . '</td>';
				$html .= '  <td>' . $value['campaign_name'] . '</td>';
				$html .= '  <td>' . $value['position_name'] . '</td>';
				$html .= '  <td>' . $value['company_name'] . '</td>';
				$html .= '  <td>' . $value['dpt_name'] . '</td>';
				$html .= '  <td style="font-size: 1.2rem">( ' . get_candidate_by_capmapign($value['id']) . ' )</td>';
				$html .= '  <td>';
				$html .= '     <a href="' . admin_url('recruitment/triagem/' . $value['id']) . '"> <button class="btn btn-primary"><i class="fa fa-eye"></i></button></a>';
				$html .= '  </td>';
				$html .= '</tr>';
			}
			$html .= '</tbody>';
			...
			$html = ' <span class="text-ranger">Sem dados, por faor, seleciona um departamento que tenha dados!</span>';
		}
		return $html;
	}
	...
	public function dashboard()
	{
		...
		$data['proporcao_genero'] = $this->recruitment_model->get_proporcao_diversidade_genero();
		$data['numero_candidatos_por_campanha'] = $this->recruitment_model->get_numero_candidatos_por_campanha();
		$data['total_candidatos_por_status'] = $this->recruitment_model->get_total_candidatos_por_status();
		$data['candidatos_status'] = $this->recruitment_model->get_candidatos_status();
		$data['campanhas_com_mais_candidatos'] = $this->recruitment_model->get_top_campanhas_com_mais_candidatos();
		
		$data['total_candidatos'] = $this->recruitment_model->get_total_candidatos();
		$data['total_entrevistas'] = $this->recruitment_model->get_total_entrevistas();

		$data['total_candidatos_por_status_teste'] =json_encode( ["teste1","teste2","teste3"]);
		// echo "<pre>";
		// var_dump($data['total_candidatos_por_status']);
		// die();
		// return;
	...
	public function skill()
	{
		if ($this->input->post()) {
			$message = '';
			$data = $this->input->post();
			$this->form_validation->set_rules('skill_name', 'Nome da habilidade', 'required', ['required' => 'Preencha o campo {field}']);
			if (!$this->form_validation->run()) {
				set_alert('danger', 'Preencha todos os campos obrigatórios');
				return redirect(admin_url('recruitment/setting?group=skills'));
			}
			if (!$this->input->post('id')) {
	...
	public function industry()
	{
		if ($this->input->post()) {
			$message = '';
			$data = $this->input->post();
			$this->form_validation->set_rules('industry_name', 'Nome da indústria', 'required', ['required' => 'Preencha o campo {field}']);
			if (!$this->form_validation->run()) {
				set_alert('danger', 'Preencha todos os campos obrigatórios');
				return redirect(admin_url('recruitment/setting?group=industry_list'));
			}
			if (!$this->input->post('id')) {
