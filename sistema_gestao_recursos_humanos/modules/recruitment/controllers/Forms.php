...
class Forms extends ClientsController
{
    ...
    public function wtl($rec_campaignid,$key)
    {
        ...
        $vf = $this->recruitment_model->verificar_candidatura_exist($rec_campaignid);
        if ($vf != 0) {
            set_alert('danger', 'Candidatura já Enviada');
            return redirect(site_url('recruitment/recruitment_portal/minhas_candidaturas'));
        }
    ...
    public function add_candidate_form_recruitment_channel($form_key) {
        $data = $this->input->post();
        if ($data) {
            $vf = $this->recruitment_model->verificar_candidatura_exist($data['rec_campaignid']);
            if ($vf != 0) {
                set_alert('warning', 'Candidatura já Enviada');
                return redirect(site_url('recruitment/recruitment_portal/minhas_candidaturas'));
            }
