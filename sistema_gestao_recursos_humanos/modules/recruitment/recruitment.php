...
function recruitment_module_init_menu_items() {
    ...
    // Verifica se o usuário tem permissão para ver o módulo de recrutamento
    if (has_permission('recruitment', '', 'view')) {
        // Obtém a rota atual
        $current_class = $CI->router->fetch_class();
        $current_method = $CI->router->fetch_method();

        // Define as rotas que correspondem ao módulo de recrutamento
        $recruitment_routes = [
            'recruitment',
            'dashboard',
        ...
        // Verifica se a rota atual pertence ao módulo de recrutamento, mas não a página inicial
        if (in_array($current_class, $recruitment_routes) && $current_class != 'dashboard') {
            ...
            $CI->app_menu->add_sidebar_children_item('recruitment', [
                'slug' => 'rec_triagem',
                'name' => _l('Triagem e Filtros'), 
                'icon' => 'fa fa-list',
                'href' => admin_url('recruitment/triagem_geral'),
                'position' => 5,
            ]);
            ...
            $CI->app_menu->add_sidebar_children_item('recruitment', [
                ...
                'name' => _l('Portal de Recrutamento'),
