...
class Recruitment_model extends App_Model
{
	...
	public function get_tipo_sanguineo () {
        return $this->db->get('rec_tipo_sanguineo')->result_array();
    }

	public function get_motivo()
    {
        return $this->db->get('rec_motivo')->result_array();
    }

    public function create_motivo($data)
    {
        $vf_existe = $this->db->get_where('rec_motivo', ['motivo' => $data['motivo']])->row_array();
        if ($vf_existe) {
            set_alert('danger', "Motivo já foi cadastrada");
            redirect('recruitment/setting?group=motivos');
        }
        return $this->db->insert('rec_motivo', $data);
    }
    public function first_motivo($id)
    {
        $query = $this->db->get_where('rec_motivo', ['id' => $id])->row_array();
        if (!$query) {
            show_404();
            exit;
        }
    ...
    public function update_motivo($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update('rec_motivo', $data);

        return $this->first_motivo($id);
    }
    public function delete_motivo($id)
    {
        $first = $this->first_motivo($id);

        $this->db->where('id', $id);
        $this->db->delete('rec_motivo');
        return $first;
    }
	...
	public function approve_reject_proposal($type, $id)
	{
		if ($type == 'approved') {
			...
			if ($this->db->affected_rows() > 0) {
				try {
					$this->db->where('id', $id);
					$result = $this->db->get(db_prefix() . 'rec_proposal')->row();
					$data['campaign_code']           = substr(md5(date('Y-m-d H:i:s')), 0, 8);
					$data['campaign_name']    		 = $result->proposal_name;
					$data['cp_proposal']			 = $id;
					$data['cp_position']       		 = $result->position;
					$data['cp_department']    		 = $result->department;
					$data['cp_amount_recruiment']    = $result->amount_recruiment;
					$data['cp_form_work']     		 = $result->form_work;
					$data['cp_workplace']     		 = $result->workplace;
					$data['cp_salary_from']     	 = $result->salary_from;
					$data['cp_salary_to']    		 = $result->salary_to;
					$data['cp_from_date']   	     = $result->from_date;
					$data['cp_to_date']    			 = $result->to_date;
					$data['cp_reason_recruitment']   = $result->reason_recruitment;
					$data['cp_job_description']      = $result->job_description;
					$data['cp_ages_from']  		     = $result->ages_from;
					$data['cp_ages_to']    		     = $result->ages_to;
					$data['cp_gender']    			 = $result->gender;
					$data['cp_height']    			 = $result->height;
					$data['cp_weight']   		     = $result->weight;
					$data['cp_literacy']     		 = $result->literacy;
					$data['cp_experience']   	     = $result->experience;
					$data['cp_add_from']     		 = $result->add_from;
					$data['cp_date_add']    		 = $result->date_add;
					$data['cp_manager']     		     = get_staff_user_id();
					$data['cp_follower']     		     = get_staff_user_id();

					$this->add_recruitment_campaign_from_proposal($data);
					return 'approved';
				} catch (\Throwable $th) {
					return false;
				}
	...
	public function add_recruitment_campaign_from_proposal($data)
	{
	...
	public function get_rec_campaign_triagem($id = '')
	{
		if ($id != '') {
	...
	public function get_rec_campaign_triagem_by_dpt($id = '')
	{
		if ($id != '') {

			$this->db->select('cp.cp_id as id, cp.campaign_code as campaign_code, cp.campaign_name as campaign_name,dpt.name as dpt_name,company.company_name as company_name,job.position_name as position_name ');
			$this->db->from(db_prefix() . 'rec_campaign as cp');
			$this->db->join(db_prefix() . 'departments as dpt', 'dpt.departmentid = cp.cp_department');
			$this->db->join(db_prefix() . 'rec_job_position as job', 'job.position_id = cp.cp_position', 'left');
			$this->db->join(db_prefix() . 'rec_company as company', 'company.id = cp.company_id', 'left');
			$this->db->where('cp.cp_status', 3);
			$this->db->where('cp.cp_department', $id);
	...
	public function add_candidate($data) {
		...
		$insert_id = $this->db->insert_id();
		if ($insert_id) {
			$this->update_prefix_number(['candidate_code_number' =>  (int) get_option('candidate_code_number')+1]);
	...
	public function get_candidates_triagem($id = '')
	{
		if ($id == '') {
			$candidate = [];
		} else {
			$this->db->where('rec_campaign', $id);
			$candidate = $this->db->get(db_prefix() . 'rec_candidate')->result_array();
		}
		return $candidate;
	}
	...
	public function first_candiadato () {
		if (!is_candidate_logged_in()) {
            show_404();
        }
		$candidato = is_candidate_logged_in();
		$query = $this->db->get_where('rec_candidate', ['id' => $candidato])->row_array();
		if (!$query) {
            show_404();
        }
		return $query;
	...
	public function vf_candidato($campaign = NULL){
		if (!is_candidate_logged_in()) {
            show_404();
        }
		$candidato = is_candidate_logged_in();
		...
		return [
			// 'id' => $query['id'],
			'rec_campaign' => $campaign ?? $query['rec_campaign'],
			'candidate_code' => $query['candidate_code'],
			'candidate_name' => $query['candidate_name'],
			// 'candidate_code' => $this->create_code('candidate_code'),
			'birthday' => $query['birthday'],
			'gender' => $query['gender'],
			'birthplace' => $query['birthplace'],
			'home_town' => $query['home_town'],
			'identification' => $query['identification'],
			'days_for_identity' => $query['days_for_identity'],
			'place_of_issue' => $query['place_of_issue'],
			...
			'height' => $query['height'],
			'weight' => $query['weight'],
			'introduce_yourself' => $query['introduce_yourself'],
			'phonenumber' => $query['phonenumber'],
			'email' => $query['email'],
			'skype' => $query['skype'],
			'facebook' => $query['facebook'],
			'resident' => $query['resident'],
			'current_accommodation' => $query['current_accommodation'],
			'status' => 1,
			'last_ip' => $query['last_ip'],
			'rate' => $query['rate'],
			'desired_salary' => $query['desired_salary'],
			'date_add' => date('Y-m-d'),
			'recruitment_channel' => 1,
			'skill' => $query['skill'],
			'interests' => $query['interests'],
			'linkedin' => $query['linkedin'],
			'alternate_contact_number' => $query['alternate_contact_number'],
		];
	}

	public function verificar_candidatura_exist($rec_campaign) {
		$candidato = $this->vf_candidato();

		$this->db->where('candidate_code', $candidato['candidate_code']);
		$this->db->where('rec_campaign', $rec_campaign);
        $query = $this->db->get('rec_candidate')->num_rows();
		return $query;
	}

	public function get_candidaturas () {
		$candidato = $this->vf_candidato();

		$this->db->select('
            rec_candidate.*,
            rec_campaign.*,
            rec_job_position.position_name,
        ');
        $this->db->from('rec_candidate');
        $this->db->join('rec_campaign', 'rec_campaign.cp_id = rec_candidate.rec_campaign', 'left');
        $this->db->join('rec_job_position', 'rec_job_position.position_id = rec_campaign.cp_position', 'left');
        $this->db->where('rec_candidate.candidate_code', $candidato['candidate_code']);
        return $this->db->get()->result_array();
	}
	...
	public function add_candidate_forms($data, $form_key = '')
	{
		...
		$dados_c = $this->vf_candidato($data['rec_campaign']);

		$this->db->insert(db_prefix() . 'rec_candidate', $dados_c);
		$insert_id = $this->db->insert_id();
	...
	public function portal_send_mail_to_friend($data)
	{
		$inbox['body'] = _strip_tags($data['content']);
		$inbox['body'] = nl2br_save_html($inbox['body']);
		if(send_email_recrutamento($data['email']. $data['subject'], $inbox['body']))
		{
			return true;
		}
		else {
	...
	public function get_proporcao_diversidade_genero()
	{
		$this->db->select('
        rc.gender AS genero,
        COUNT(rc.id) AS total_candidatos,
        ROUND((COUNT(rc.id) * 100.0 / (SELECT COUNT(*) FROM ' . db_prefix() . 'rec_candidate)), 2) AS proporcao_percentual
    ');
		$this->db->from(db_prefix() . 'rec_candidate rc');
		$this->db->group_by('rc.gender'); // Agrupa por gênero
		$this->db->order_by('rc.gender', 'asc'); // Agrupa por gênero

		$query = $this->db->get();
		return $query->result_array(); // Retorna os resultados em forma de array
	}
	public function get_numero_candidatos_por_campanha()
	{
		// Consulta para obter o número de candidatos por campanha
		$this->db->select('
        rc.rec_campaign AS campanha_id,
        rcp.campaign_name, 
        COUNT(rc.id) AS total_candidatos
    ');
		$this->db->from(db_prefix() . 'rec_candidate rc');
		// Relaciona a tabela rec_candidate com a tabela rec_campaign
		$this->db->join(db_prefix() . 'rec_campaign rcp', 'rc.rec_campaign = rcp.cp_id', 'left');
		$this->db->group_by('rc.rec_campaign'); // Agrupa por campanha
		$this->db->order_by('rcp.cp_date_add', 'DESC'); // Ordena pelas campanhas mais recentes
		$this->db->limit(4); // Limita a 4 últimas campanhas

		// Executa a consulta
		$query = $this->db->get()->result_array();
		return $query; // Retorna o array com o número de candidatos por campanha
	}
	public function get_total_candidatos_por_status()
	{
    ...
    ');
        ...
        $dados .=']';
		return $dados; // Retorna um array com o total de candidatos por status
	}
	public function get_candidatos_status()
	{
		// Consulta para obter o total de candidatos por status
		$this->db->select('
        rc.status AS status,
	...
	public function get_top_campanhas_com_mais_candidatos()
	{
		// Consulta para obter o total geral de candidatos
		$this->db->select('COUNT(id) AS total_geral_candidatos');
		$this->db->from(db_prefix() . 'rec_candidate');
		$total_geral_query = $this->db->get();
		$total_geral = $total_geral_query->row()->total_geral_candidatos;

		// Consulta para obter as 3 campanhas com o maior número de candidatos
		$this->db->select('
        rcp.cp_id AS campanha_id,
        rcp.campaign_name AS nome_campanha,
        COUNT(rc.id) AS total_candidatos,
        (COUNT(rc.id) / ' . $total_geral . ' * 100) AS percentual_candidatos
    ');
		$this->db->from(db_prefix() . 'rec_campaign rcp'); // Tabela de campanhas
		$this->db->join(db_prefix() . 'rec_candidate rc', 'rc.rec_campaign = rcp.cp_id', 'left'); // Relaciona com a tabela de candidatos
		$this->db->group_by('rcp.cp_id'); // Agrupa por campanha
		$this->db->order_by('total_candidatos', 'DESC'); // Ordena pelo total de candidatos em ordem decrescente
		$this->db->limit(3); // Limita o resultado às 3 campanhas com mais candidatos

		// Executa a consulta
		$query = $this->db->get();
		return $query->result_array(); // Retorna um array com as 3 campanhas, seus totais e percentuais
	}

	public function get_total_candidatos()
	{
		// Consulta para obter o número total de candidatos
		$this->db->select('COUNT(id) AS total_candidatos');
		$this->db->from(db_prefix() . 'rec_candidate'); // Tabela de candidatos

		// Executa a consulta
		$query = $this->db->get();

		// Retorna o total de candidatos
		return $query->row()->total_candidatos;
	}
	public function get_total_entrevistas()
	{
		// Consulta para obter o número total de entrevistas
		$this->db->select('COUNT(id) AS total_entrevistas');
		$this->db->from(db_prefix() . 'rec_interview'); // Tabela de entrevistas

		// Executa a consulta
		$query = $this->db->get();

		// Retorna o total de entrevistas
		return $query->row()->total_entrevistas;
	}

	public function plano_campanha($filtro = '') {
		if (!empty($filtro)) {
			$this->db->where('status', $filtro);
		}
		$query = $this->db->get('rec_proposal')->num_rows();
        return $query;
	}
	
	public function total_candidaturas($filtro = '') {
		if (!empty($filtro)) {
			$this->db->where('status', $filtro);
		}
		$query = $this->db->get('rec_candidate')->num_rows();
