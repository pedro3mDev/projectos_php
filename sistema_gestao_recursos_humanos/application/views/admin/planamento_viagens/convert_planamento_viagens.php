...
<div class="modal fade" id="convert_planamento_viagens_form" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <?php echo form_open('admin/planamento_viagens/convert/' . $estimate_request->id, ['id' => 'request_to_client_form']); ?>
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span>
                </button>
                <h4 class="modal-title" id="myModalLabel">
                    <?php echo _l('convert_planamento_viagens'); ?>
                </h4>
            </div>
            <div class="modal-body">
                <?php echo form_hidden('requestid', $planamento_viagens->id); ?>
                <?php echo form_hidden('default_language', $planamento_viagens->default_language); ?>
                <input type="hidden" name="convert_to">
                <div id="rel_wrapper">
                    <div class="form-group select-placeholder" id="rel_type_wrapper">
                        <label for="rel_type"
                            class="control-label"><?php echo _l('planamento_viagens_related'); ?></label>
                        <select name="rel_type" id="rel_type" class="selectpicker" data-width="100%"
                            data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                            <option value=""></option>
                            <option value="lead"><?php echo _l('planamento_viagens_for_lead'); ?></option>
                            <option value="customer"><?php echo _l('planamento_viagens_for_customer'); ?></option>
                        </select>
                    </div>
