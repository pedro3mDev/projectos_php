...
class Gv_reserva_hotel_model extends App_Model
{
    ...
    public function get($id)
    {
        ...
        $query = $this->db->where('pedido_viagem_id', $id)->get('gv_reserva_hotel');
    ...
    public function first($dados) {
        ...
        $query = $this->db->get_where('gv_reserva_hotel', ['id' => $dados['id'], 'pedido_viagem_id' => $dados['id_pedido']])->row_array();
        if (!$query) {
            header("HTTP/1.1 404 NOT FOUND");
            echo json_encode(['status' => false, 'message' => 'Reserva de Hotel não encontrado']);
            exit;
        }
        return $query;
    }
